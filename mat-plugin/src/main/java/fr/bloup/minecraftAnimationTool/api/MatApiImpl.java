...
public class MatApiImpl implements MinecraftAnimationToolApi {
    ...
    public AnimatedModel spawn(String name, String cacheName, Location location) {
        return spawn(name, cacheName, location, 1.0f);
    }

    @Override
    public AnimatedModel spawn(String name, String cacheName, Location location, float scale) {
        Cache c = cache(cacheName);
        if (c == null || name == null || location == null || scale <= 0) return null;
        EntityPOJO pojo = entityManager.createEntity(entityManager.uniqueName(name), c, scale);
