...
public class PauseSubCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 0) {
            commandSender.sendMessage("Usage: /mat pause <name>");
            return true;
        }
        ...
        if (entityManager.pause(entityPOJO)) {
            commandSender.sendMessage("Paused animation on " + entityPOJO.getName());
        } else {
            commandSender.sendMessage(entityPOJO.getName() + " has no active animation.");
        }
        return true;
    }
